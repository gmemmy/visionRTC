...
import { processFrame, setSourceId, clearSourceId, getFrameCount } from './VisionCameraFrameProcessor';
...
export default function TestFramework({ onBack }: TestFrameworkProps = {}) {
  ...
  const testSetupFrameProcessor = useCallback(async () => {
    ...
    // Setup the frame processor to deliver frames to our source
    setSourceId(currentSourceId);
    
    // Verify the method exists
    const NativeVisionRTC = require('react-native-vision-rtc/src/NativeVisionRtc').default;
  ...
  const testFrameDelivery = useCallback(async () => {
    ...
    while (attempts < maxAttempts) {
      ...
      if (currentFrameCount > initialFrameCount) {
        ...
        if (stats && (stats.producedFps > 0 || stats.deliveredFps > 0)) {
          console.log(`✅ Frame delivery working! Processed ${currentFrameCount} frames, Stats:`, stats);
          return;
        }
      }
    ...
    throw new Error(`No frames delivered after ${maxAttempts * 100}ms. Frame count: ${getFrameCount()}`);
  ...
  const testInvalidSourceId = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('INVALID_SOURCE_ID')) {
  ...
  const runCoreIntegrationTests = useCallback(async () => {
    await runTest('Core Integration', 'Create Vision Camera Source', testCreateVisionCameraSource);
    await runTest('Core Integration', 'Create WebRTC Track', testCreateWebRTCTrack);
    await runTest('Core Integration', 'Setup Frame Processor', testSetupFrameProcessor);
    await runTest('Core Integration', 'Test Frame Delivery', testFrameDelivery);
    await runTest('Core Integration', 'Dispose Resources', testDisposeResources);
  }, [runTest, testCreateVisionCameraSource, testCreateWebRTCTrack, testSetupFrameProcessor, testFrameDelivery, testDisposeResources]);

  const runMemoryManagementTests = useCallback(async () => {
    // Recreate source for memory tests
    await runTest('Core Integration', 'Create Vision Camera Source', testCreateVisionCameraSource);
    
    await runTest('Memory Management', 'Multiple Track Creation/Disposal', testMultipleTrackCreationDisposal);
    await runTest('Memory Management', 'Source Disposal Cleanup', testDisposeResources);
  }, [runTest, testCreateVisionCameraSource, testMultipleTrackCreationDisposal, testDisposeResources]);
  ...
  const runPerformanceTests = useCallback(async () => {
    ...
    await runTest('Performance', 'Frame Delivery Latency', testFrameDeliveryLatency);
    
    // Cleanup
    await runTest('Core Integration', 'Dispose Resources', testDisposeResources);
  }, [runTest, testCreateVisionCameraSource, testCreateWebRTCTrack, testFrameDeliveryLatency, testDisposeResources]);
  ...
  }, [isRunning, runCoreIntegrationTests, runMemoryManagementTests, runPerformanceTests, runErrorHandlingTests]);
