...
class VisionRTC: NSObject {
  ...
  func disposeTrack(trackId: NSString, resolver: RCTPromiseResolveBlock, rejecter: RCTPromiseRejectBlock) {
    stateQueue.async(flags: .barrier) {
      ...
      // Clean up retained pixel buffer to prevent memory leaks
      self.latestBufferByTrack.removeValue(forKey: trackId as String)
      // Clean up statistics to prevent memory leaks
...
extension VisionRTC {
  fileprivate func deliverExternalFrame(sourceId: String, pixelBuffer: CVPixelBuffer, timestampNs: Int64) {
    ...
    for trackId in trackIds {
      // Batch all reads in a single sync call for better performance
      var isPaused = false
      var stOpt: TrackState?
      var srcOpt: RTCVideoSource?
      var capOpt: RTCVideoCapturer?
      ...
      // Use a single sync operation for both read and write to avoid deadlocks
      stateQueue.sync {
        let last = self.lastSent[trackId] ?? 0
      ...
      // Validate frame before delivery
